"""Aggregates Illinois-located listings from every taildraggers.com
...
ones whose location ends in ", IL".
...
California/Colorado/Connecticut/Delaware/Florida/Georgia/Hawaii/Idaho repos'
approach, with the state filter swapped to "IL".
"""
...
PAGE_TITLE = "Taildragger Ads in Illinois"
STATE_ABBR = "IL"
...
_ILLINOIS_LOCATION_RE = re.compile(r",\s*IL\s*$", re.IGNORECASE)


def _is_illinois(location: str) -> bool:
    return bool(_ILLINOIS_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        illinois_rows = [row for row in rows if _is_illinois(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(illinois_rows)} in Illinois")
        all_listings.extend(illinois_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Illinois listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Illinois listings: {len(listings)}")
