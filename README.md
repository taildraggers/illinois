...
Daily aggregator of Illinois-located taildragger classified listings,
...
[Idaho](https://github.com/taildraggers/idaho) repos' approach, with
the state filter swapped to "IL".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, IL` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Illinois"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/illinois/`
...
```html
<iframe
  src="https://taildraggers.github.io/illinois/"
  title="Taildragger Ads in Illinois"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_ILLINOIS_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
